...
from api.utils.enrollment import create_enrollment, get_enrollment_by_class_id
...
@router.get("/enrollment/{id}", response_model=List[Enrollment])
async def get_enrollment_by_id_api(id: int, db: Session = Depends(get_db)):
    student = get_enrollment_by_class_id(db, class_id=id)
    if student == None:
        raise HTTPException(status_code=404, detail="Student Not found")
    return student
