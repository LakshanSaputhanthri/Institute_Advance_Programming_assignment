...
from api.utils.attendance import create_attendance
...
# async def get_attendance_api(skip:int=0,limit:int=100,db:Session=Depends(get_db)):
#     students=get_att(db,skip=skip,limit=limit)
#     return students
...
async def create_attendance_api(attendance:AttendanceCreate,db:Session=Depends(get_db)):
